...
class VAEtrimodalSVHNMNIST(MultimodalVAE):
    ...
    def get_random_styles(self, num_samples):
        if self.flags.factorized_representation:
            z_style_1 = torch.randn(num_samples, self.flags.style_m1_dim, device=self.flags.device);
            z_style_2 = torch.randn(num_samples, self.flags.style_m2_dim, device=self.flags.device);
            z_style_3 = torch.randn(num_samples, self.flags.style_m3_dim, device=self.flags.device);
        else:
            z_style_1 = None;
            z_style_2 = None;
            z_style_3 = None;
    ...
    def save_networks(self, save_path: str):
        torch.save(self.encoder_m1.state_dict(), os.path.join(save_path, self.flags.encoder_save_m1))
        torch.save(self.decoder_m1.state_dict(), os.path.join(save_path, self.flags.decoder_save_m1))
        torch.save(self.encoder_m2.state_dict(), os.path.join(save_path, self.flags.encoder_save_m2))
        torch.save(self.decoder_m2.state_dict(), os.path.join(save_path, self.flags.decoder_save_m2))
        torch.save(self.encoder_m3.state_dict(), os.path.join(save_path, self.flags.encoder_save_m3))
        torch.save(self.decoder_m3.state_dict(), os.path.join(save_path, self.flags.decoder_save_m3))
