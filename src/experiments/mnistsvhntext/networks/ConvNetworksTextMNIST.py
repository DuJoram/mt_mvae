...
class EncoderText(Encoder):
    def __init__(self, flags):
        ...
        if flags.factorized_representation:
            ...
            # class
            self.class_mu = nn.Linear(in_features=2*flags.dim, out_features=class_dim, bias=True)
            self.class_logvar = nn.Linear(in_features=2*flags.dim, out_features=class_dim, bias=True)
        else:
            #non-factorized
            self.latent_mu = nn.Linear(in_features=2*flags.dim, out_features=class_dim, bias=True)
            self.latent_logvar = nn.Linear(in_features=2*flags.dim, out_features=class_dim, bias=True)
