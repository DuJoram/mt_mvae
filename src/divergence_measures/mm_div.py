...
def poe_stable(means, log_vars, eps=None):
    log_precisions = -log_vars
    precisions = log_precisions.exp()
    joint_log_precision = torch.logsumexp(log_precisions, dim=0)
    joint_precision = joint_log_precision.exp()

    joint_log_var = -joint_log_precision
    joint_var = joint_log_var.exp()
    joint_mean = precisions.mul(means).sum(dim=0) * joint_var
...
def alpha_poe(alpha, mu, logvar, eps=1e-8):
    ...
    T = 1 / var
    pd_var = 1.0 / torch.sum(alpha_expanded * T, dim=0)
...
def calc_alphaJSD_modalities_mixture(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    ...
    for k in range(0, len(mus)):
        ...
        kld_lb = calc_kl_divergence_lb_gauss_mixture(
            flags, k, mus[k], logvars[k], mus, logvars, norm_value=flags.batch_size
        )
        print("kld_lb: " + str(kld_lb))
        # print('ub: ')
        kld_ub = calc_kl_divergence_ub_gauss_mixture(
            flags, k, mus[k], logvars[k], mus, logvars, ent, norm_value=flags.batch_size
        )
...
def calc_alphaJSD_modalities(flags, mus, logvars, weights, normalization=None):
    ...
    if normalization is not None:
        klds = torch.zeros(num_mods, device=flags.device)
    else:
        klds = torch.zeros(num_mods, num_samples, device=flags.device)
...
def calc_group_divergence_poe(flags, mus, logvars, norm=None):
    num_mods = mus.shape[0]
    if flags.stable_poe:
        poe_mu, poe_logvar = poe_stable(mus, logvars)
    else:
        poe_mu, poe_logvar = poe(mus, logvars)
    kld_poe = calc_kl_divergence(poe_mu, poe_logvar, norm_value=norm)
    klds = torch.zeros(num_mods, device=flags.device)
    for k in range(0, num_mods):
        kld_ind = calc_kl_divergence(mus[k, :, :], logvars[k, :, :], norm_value=norm)
        klds[k] = kld_ind
    ...
    if flags.modality_poe:
        ...
        for i in range(1, len(mus)):  # CAREFUL: index starts from one, not zero
            for j in range(0, len(mus)):
                kld = calc_kl_divergence(
                    mus[i], logvars[i], mus[j], logvars[j], norm_value=flags.batch_size
                )
                klds[i, j] = kld
                if i >= 1 and j >= 1:
