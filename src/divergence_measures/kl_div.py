...
def calc_kl_divergence(mu0, logvar0, mu1=None, logvar1=None, norm_value=None):
    ...
    if mu1 is None or logvar1 is None:
        KLD = -0.5 * torch.sum(1 - logvar0.exp().add(1e-4) - mu0.pow(2) + logvar0)
    else:
        KLD = -0.5 * (torch.sum(1 - logvar0.exp().add(1e-4)/logvar1.exp().add(1e-4) - (mu0-mu1).pow(2)/logvar1.exp().add(1e-4) + logvar0 - logvar1))
