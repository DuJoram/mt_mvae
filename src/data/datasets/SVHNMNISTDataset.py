...
class VisionDataset(MultimodalDataset, abc.ABC):
    _repr_indent = 4
...
class SVHNMNIST(VisionDataset):
    ...
    def __init__(self, flags,  alphabet, font, train=True, transform=None, target_transform=None):
        ...
        self._font = font
        self._plot_image_size = torch.Size((3, 28, 28))
        ...
        self._modalities = [
            MNIST(),
            SVHN(self._plot_image_size),
            Text(self.len_sequence, self.alphabet, self._plot_image_size, self._font)
        ]

        shared_attributes = ["digit"]
        self._attributes: List[List[str]] = [shared_attributes for _ in self._modalities]
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            ...
            target = target_mnist;
        target_dict = {'mnist': target, 'svhn': target, 'text': target}
