...
def calc_inception_features(exp, dims=2048, batch_size=128):
    ...
    inception_state_dict = exp.flags.inception_state_dict
    dir_gen_eval_fid = exp.flags.dir_gen_eval_fid


    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]

    model = InceptionV3([block_idx], path_state_dict=inception_state_dict)
    model = model.to(device)

    for modality in modalities:
        if modality.gen_quality_eval:
            for k, key in enumerate(paths.keys()):
                if key == '':
                    continue
                if key == 'random' or key == 'real':
                    dir_gen = paths[key];
                    if not os.path.exists(dir_gen):
                        raise RuntimeError('Invalid path: %s' % dir_gen)
                    files_gen = glob.glob(os.path.join(dir_gen, modality.name, '*' +
                                                       modality.file_suffix))
                    fn = os.path.join(dir_gen_eval_fid,
                                      key + '_' + modality.name + '_activations.npy');
...
def calculate_inception_features_for_gen_evaluation(flags, paths, modality=None, dims=2048, batch_size=128):
    dir_gen_eval_fid_random = flags.dir_gen_eval_fid_random
    inception_state_dict = flags.inception_state_dict
    dir_gen_eval_fid_cond_gen = flags.dir_gen_eval_fid_cond_gen
    dir_gen_eval_fid_real = flags.dir_gen_eval_fid_real
    ...
    if 'random' in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_random = os.path.join(dir_gen_eval_fid_random, 'random_sampling_' + modality + '_activations.npy');
        else:
            files_rand_gen = glob.glob(os.path.join(dir_rand_gen, '*.png'));
            filename_random = os.path.join(dir_gen_eval_fid_random, 'random_img_activations.npy')
    ...
    if 'conditional' in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_conditional = os.path.join(dir_gen_eval_fid_cond_gen, 'conditional_img_activations.npy')
    ...
    if 'real' in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_real = os.path.join(dir_gen_eval_fid_real, 'real_' + modality + '_activations.npy');
        else:
            files_real = glob.glob(os.path.join(dir_real, '*.png'));
            filename_real = os.path.join(dir_gen_eval_fid_real, 'real_img_activations.npy')
...
def get_classifiers_activations(flags, data, model):
    model.eval();
    ...
    for modality in modalities:
        if modality.gen_quality_eval:
            for k, key in enumerate(exp.subsets):
                ...
                #                        acts[mod.name][key]);
                ap_prd = calculate_fid(acts[modality.name]['real'],
                                       acts[modality.name][key])
                ap_prds[key + '_' + modality.name] = ap_prd

    for modality in modalities:
        if modality.gen_quality_eval:
            ap_prd = calculate_fid(acts[modality.name]['real'],
                                   acts[modality.name]['random']);
            ap_prds['random_' + modality.name] = ap_prd;
    return ap_prds;
