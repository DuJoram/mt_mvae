...
def get_latent_samples(flags, latents, n_imp_samples, modalities=None):
    l_c = latents['content'];
    l_s = latents['style'];
...
def log_joint_estimate(flags, n_samples, likelihoods, targets, styles, content, dynamic_prior=None):
    ...
    log_px_zs = torch.zeros(num_mods, batch_size * n_samples, device=flags.device);
