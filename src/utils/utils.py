...
def get_likelihood(str):
    if str == 'laplace':
        ...
        pz = dist.Bernoulli;
    elif str == 'bernoulli-logits':
        pz = lambda *logits: dist.Bernoulli(logits=logits[0])
    elif str == 'normal':
        pz = dist.Normal;
...
def reparameterize(mu, logvar, std_coef=1.):
    std = logvar.mul(0.5).exp() * std_coef
...
def mixture_component_selection(flags, mus, logvars, w_modalities=None, num_samples=None, subset_names=None):
    #if not defined, take pre-defined weights
    num_components = mus.shape[0];
    num_samples = mus.shape[1];
    if w_modalities is None:
        w_modalities = torch.tensor(flags.alpha_modalities, device=flags.device)
    idx_start = [];
    idx_end = []
    ...
    if subset_names is None:
        ...
        subsets = [[subset_names[k]]*(idx_end[k]-idx_start[k]) for k in range(w_modalities.shape[0])]
        subsets = [x for lst in subsets for x in lst]
        return mu_sel, logvar_sel, subsets
...
def calc_elbo(exp, modality_name, recs, klds, weighted=True):
    flags = exp.flags;
    ...
    if modality_name == 'joint':
        ...
        for modality in modalities:
            if flags.factorized_representation:
                if not weighted:
                    ...
                    s_w = s_weights[modality.name]
                w_style_kld += s_w * klds_style[modality.name];
            w_rec += r_weights[modality.name] * recs[modality.name];
        ...
        if not weighted:
            ...
            beta_style_mod = s_weights[modality_name];
        rec_weight_mod = r_weights[modality_name];
        kld_style = beta_style_mod * klds['style'][modality_name];
        rec_error = rec_weight_mod * recs[modality_name];
    if not weighted:
        beta_s = 1.0
        beta_c = 1.0
        beta = 1.0
    else:
        beta_s = flags.beta_style
        beta_c = flags.beta_content
        beta = flags.beta
    div = beta_c * kld_content + beta_s * kld_style;
    elbo = rec_error + beta * div;
    return elbo;
